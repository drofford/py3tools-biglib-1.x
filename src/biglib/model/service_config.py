...
class ServiceConfig:
    ...
    def load_from_file(cls, file_or_file_path):
        logger.debug(f"load_from_file: {type(file_or_file_path)=}")
        ...
        if isinstance(file_or_file_path, io._io.TextIOWrapper):
            ...
            try:
                ...
                logger.debug(f"load_from_file: {err}")
                return False, [err]
        ...
        try:
            logger.debug(f"load_from_file: calling json.load")
            ot = json.load(handle)
            svc_cfg = cls.parse_dict_to_service_config(ot)
            logger.debug(f"load_from_file: [a] {svc_cfg=}")
            outcome = True

        except Exception as ex:
            logger.debug(f"{type(ex)=}")
            svc_cfg = [str(ex)]
            logger.debug(f"load_from_file: [b] {svc_cfg=}")

        logger.debug(f"load_from_file: returning")
        logger.debug(f"load_from_file:     {outcome=}")
        logger.debug(f"load_from_file:     {svc_cfg=}")

        return outcome, svc_cfg

    @classmethod
    def parse_dict_to_service_config(cls, ot) -> ServiceConfig:
        logger.debug(f"parse_dict_to_service_config: {type(ot)=}")
